...
for i in range(iterNum):
    ...
    while simTime >= dt * step:
        ...
        if step > 5000:
            ...
            relaYaw01errRandInput[i].append(
                np.abs(xEsti[2, 1] - xTrueRL[2, 1] - 3.14 * round((xEsti[2, 1] - xTrueRL[2, 1]) / 3.14)))
...
for i in range(iterNum):
    ...
    while simTime >= dt * step:
        ...
        if step > 5000:
            ...
            relaYaw01errFormationInput[i].append(
                np.abs(xEsti[2, 1] - xTrueRL[2, 1] - 3.14 * round((xEsti[2, 1] - xTrueRL[2, 1]) / 3.14)))
...
for i in range(iterNum):
    ...
    while simTime >= dt * step:
        ...
        if step > 5000:
            ...
            relaYaw01errV1zeroInput[i].append(
                np.abs(xEsti[2, 1] - xTrueRL[2, 1] - 3.14 * round((xEsti[2, 1] - xTrueRL[2, 1]) / 3.14)))
...
plt.legend([bplot1["boxes"][0], bplot1["boxes"][1], bplot1["boxes"][2]], ['Random', 'Formation', '$v_j=0$'],
           fontsize=11)  # bbox_to_anchor=(0.8, 1)
plt.show()
