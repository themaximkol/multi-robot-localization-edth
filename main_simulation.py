...
np.random.seed(514)  # seed the random number generator for reproducibility
border = {"xmin": -4, "xmax": 4, "ymin": -4, "ymax": 4, "zmin": 0, "zmax": 4}
numRob = 8  # number of robots
dt = 0.01  # time interval [s]
simTime = 40.0  # simulation time [s]
maxVel = 3  # maximum velocity [m/s]
devInput = np.array(
    [[0.25, 0.25, 0.01]]
).T  # input deviation in simulation, Vx[m/s], Vy[m/s], yawRate[rad/s]
...
def animate(step):
    ...
    if step * dt < 10:
        ...
        xEsti = transform.calcAbsPosUseRelaPosWRTRob0(
            xTrue[:, 0], relativeState, xTrue, numRob
        )
        ...
        circle.radius = 0  # zNois[0, 1] # plot a circle to show the distance between robot 0 and robot 1
        time_text.set_text("t={:.2f}s, distance={:.2f}".format(step * dt, distance))
    else:
        u, final_positions = data.come_to_position(xTrue, relativeState)
        # u = data.calcInput_PotentialField(step, xTrue)
        # u = data.calcInput_Formation01(step, relativeState)
        xTrue, zNois, uNois = data.update(xTrue, u)
        if step % ekfStride == 0:
        ...
        # pointsReference.set_data(
        #     final_positions[:, 0], final_positions[:, 1]
        # )  # plot final positions
        circle.center = (xTrue[0, 0], xTrue[1, 0])
        circle.radius = 0  # zNois[0, 1] # plot a circle to show the distance between robot 0 and robot 1
        time_text.set_text("t={:.2f}s, error={:.2f}".format(step * dt, distance))
    return (
        pointsTrue,
        pointsEsti,
        circle,
...
if show_animation:
    ...
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    title = ax.set_title("2D Sward")
    (pointsTrue,) = ax.plot(
        [], [], linestyle="", marker="o", color="b", label="True Position"
    )
    (pointsEsti,) = ax.plot(
        [], [], linestyle="", marker="o", color="r", label="Predicted Position"
    )
    (pointsTrueHead,) = ax.plot([], [], linestyle="", marker=".", color="g")
    (pointsEstiHead,) = ax.plot([], [], linestyle="", marker=".", color="g")
    # (pointsReference,) = ax.plot(
    #     [], [], linestyle="", marker="x", color="k", label="Reference Robot 0"
    # )
    ax.legend()
    ...
    ax3.grid(True)
    ax3.legend(
        loc="upper center", bbox_to_anchor=(0.8, 0.6), shadow=True, ncol=1, fontsize=12
    )
